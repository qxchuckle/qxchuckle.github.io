document.addEventListener('DOMContentLoaded', function () {
  ...
  const scrollFn = function () {
    ...
    const updateHeaderClass = () => {
      ...
      if (currentTop > 0 && !isHeaderFixed) {
        classList.add('nav-fixed');
        isHeaderFixed = true;
      } else if (currentTop === 0 && isHeaderFixed) {
        classList.remove('nav-fixed');
        isHeaderFixed = false;
      }
    }
    updateHeaderClass();
    // 使用节流函数来限制事件触发频率为200ms
    window.removeEventListener('scroll', window.throttledScroll);
    window.throttledScroll = btf.throttle(updateHeaderClass, 100);
