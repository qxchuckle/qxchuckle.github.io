...
function allAI() {
  ...
  //tianliGPT的参数
  const tlReferer = 'https://www.qcqx.cn/';
  const tlKey = 'tN2jLmG7fX9zHk5dVbQr';
  const post_title =  document.querySelector('#post>#post-info>.post-title').textContent;
  //-----------------------------------------------
  ...
  function recommendList() {
    if(sessionStorage.getItem('recommendList')){
      return sessionStorage.getItem('recommendList');
    }
    let info = `推荐文章：<br />`;
    ...
    info += '</div>'
    sessionStorage.setItem('recommendList', info);
    return info;
  }
  //ai首屏初始化，绑定按钮注册事件
      ...
      try {
        response = await fetch('https://summary.tianli0.top/', {
          ...
          body: JSON.stringify({
            content: content,
            key: tlKey,
            title: post_title ? post_title : document.title,
